import random
import numpy as np
import tensorflow._api.v2.compat.v1 as tf

"""
这是书上的版本（书P516），强行通过兼容包来运行
使用正弦序列预测，为了知道RNN的训练过程
后面的数据流图查看，查看书P343
"""
tf.disable_eager_execution()  # 禁用急切执行，从此使用会话来获取操作结果
truncated_backprop_length = 10  # 反向传播截断序列长度
state_size = 10
batch_size = 10


def generate_data(n):
    """
    产生2000组正弦数

    :param n: 每组数组的长度
    :return:1500个训练集，500个验证集
    """
    x_seq = []
    y_seq = []
    for i in range(2000):
        k = random.uniform(1, 50)  # 生成实数在1-50之间
        x_1 = [np.sin(k + j) for j in range(0, n)]
        y_1 = [np.sin(k + n)]
        x_seq.append(x_1)
        y_seq.append(y_1)

    # 分割模型，前面1500用于训练，后面500用于验证
    train_x_1 = np.array(x_seq[0: 1500])
    train_y_1 = np.array(y_seq[0: 1500])
    test_x_1 = np.array(x_seq[1500:])
    test_y_1 = np.array(y_seq[1500:])
    return train_x_1, train_y_1, test_x_1, test_y_1


(train_x, train_y, test_x, test_y) = generate_data(truncated_backprop_length)
# 占位符
X_placeholder = tf.placeholder(tf.float32, [None, truncated_backprop_length])
Y_placeholder = tf.placeholder(tf.float32, [None, 1])

init_state = tf.placeholder(tf.float32, [batch_size, state_size])
# 输入至隐含层权值
W = tf.Variable(np.random.rand(state_size + 1, state_size), dtype=tf.float32)
# 隐含层阈值
b = tf.Variable(np.zeros((1, state_size)), dtype=tf.float32)
# 隐含至输出层权值
W2 = tf.Variable(np.random.rand(state_size, 1), dtype=tf.float32)
# 输出层阈值
b2 = tf.Variable(np.zeros(1), dtype=tf.float32)
# 开始前向传播（计算隐含层）
inputs_series = tf.unstack(X_placeholder, axis=1)  # 将数据拆分成列

current_state = init_state
for current_input in inputs_series:
    current_input = tf.reshape(current_input, [batch_size, 1])  # 将列表变成张量
    input_and_state_concatenated = tf.concat([current_input, current_state], 1)  # 连接两个张量，使之成为W
    next_state = tf.tanh(tf.matmul(input_and_state_concatenated, W) + b)
    current_state = next_state

logits = tf.matmul(current_state, W2) + b2
loss = tf.square(tf.subtract(Y_placeholder, logits))
train_step = tf.train.AdamOptimizer(1e-4).minimize(loss)
# 参数训练与模型评估
with tf.Session() as sess:
    sess.run(tf.global_variables_initializer())
    _current_state = np.zeros((batch_size, state_size))

    for epoch_id in range(500):
        for batch_id in range(len(train_x) // batch_size):
            begin = batch_id * batch_size
            end = begin + batch_size

            batchX = train_x[begin:end]
            batchY = train_y[begin:end]

            _train_step, _current_state = sess.run(
                [train_step, current_state],
                feed_dict={
                    X_placeholder: batchX,
                    Y_placeholder: batchY,
                    init_state: _current_state
                })

        test_indices = np.arange(len(test_x))
        np.random.shuffle(test_indices)
        test_indices = test_indices[0: 10]

        x = test_x[test_indices]
        y = test_y[test_indices]
        val_loss = np.mean(sess.run(
            loss,
            feed_dict={
                X_placeholder: x,
                Y_placeholder: y,
                init_state: _current_state
            }))
        print('epoch: %s' % epoch_id, ', loss: %g' % val_loss)
    # 数据流图可视化,在终端执行：tensorboard --logdir=./my_graph/1
    writer = tf.summary.FileWriter('../my_graph/1')
    writer.add_graph(sess.graph)
